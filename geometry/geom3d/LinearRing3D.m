...
methods
    ...
    function varargout = draw(varargin)
        ...
        h = zeros(numel(obj), 1);
        for i = 1:numel(obj)
            % add index to vertex vertex in order to draw a loop
            inds = [1:size(obj(i).Coords, 1) 1];
            h(i) = plot3(ax, obj(i).Coords(inds,1), obj(i).Coords(inds,2), obj(i).Coords(inds,3), varargin{:});
