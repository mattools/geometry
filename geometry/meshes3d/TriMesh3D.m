...
methods
    ...
    function [dist, proj] = distance(obj, point, varargin)
        % Shortest distance between a Point3D and this mesh.
        if isa(point, 'Point3D')
            [dist, proj] = distanceToPoint(obj, [point.X point.Y point.Z]);
        elseif isa(point, 'MultiPoint3D')
            [dist, proj] = distanceToPoint(obj, point.Coords);
        end
    end
    
    function [dist, proj] = distanceToPoint(obj, point, varargin)
        % Shortest distance between 3D point coordinates and the mesh.
        %
